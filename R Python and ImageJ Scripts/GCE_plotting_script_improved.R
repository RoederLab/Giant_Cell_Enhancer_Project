...
#Perform log-transformed ANOVA and Tukey HSD Test on subsetted data
Signif_dofs = aov(log(Small.Giant_ratio) ~ Genotype, data = Dofs)
summary(Signif_dofs)
hsd2 = HSD.test(Signif_dofs, "Genotype")

#group data by genotype and extract the significance letters from HSD test
Signif_summary2 = Dofs %>% group_by(Genotype) %>% summarize(Mean.ratio=mean(Small.Giant_ratio))
signif_letters2 = data.frame(row.names(hsd2$groups),hsd2$groups$groups,hsd2$groups$`log(Small.Giant_ratio)`)

#reorder the the significance letters by alphabetical order of the genotypes so the order is the same as the plot
signif_letters2 = signif_letters2 %>% arrange(signif_letters2$hsd2.groups..log.Small.Giant_ratio..)
...
#t.test(Small.Giant_ratio ~ Genotype, data = subset(Trihelix, Genotype %in% c("pLH166","pLH167"))) #.0003 **
